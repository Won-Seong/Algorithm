...
int LCA::LeastCommonAnc(int left, int right) const
{
	while (depth_table[left] != depth_table[right]) {
		if (depth_table[left] > depth_table[right])
			left = depth_matrix[left][std::floor(std::log2(depth_table[left] - depth_table[right]))];
		else
			right = depth_matrix[right][std::floor(std::log2(depth_table[right] - depth_table[left]))];
	}

	if (left == right) return left;

	for (int i = std::floor(std::log2(depth_table[left])); i >= 0; i--)
	{
...
void LCA::SimpleDFS(Vector& visit_table ,int now, int depth) 
{
	...
	for (const auto& itr : matrix_[now]) {
		if (visit_table[itr] == 0) {
			visit_table[itr] = 1;
			depth_matrix[itr][0] = now;
			SimpleDFS(visit_table,itr, depth + 1);
		}
	}
}
