...
private:
	...
	private://Position 관련 내부 편의 함수
		...
		bool HasLeft(const Position& input_position) const { return static_cast<int>(2 * index(input_position)) <= size(); }//만약 해당 Node가 left_child를 갖고 있다면 Size는 그보다 클 것이다
		bool HasRight(const Position& input_position) const { return static_cast<int>(2 * index(input_position) + 1) <= size(); }//만약 해당 Node가 right_child를 갖고 있다면 Size는 그보다 클 것이다
